...
interface BaybayProps {
  columns: TableColumn<Period>[];
}

const Baybay: React.FC<BaybayProps> = ({ columns }) => {
  const currentYear = new Date().getFullYear().toString();
  const [selectedYear, setSelectedYear] = useState<string>(currentYear);

  const timeline = useHarvestTimeline(
    new Date("2025-08-04"),
    7,
    ["elna", "perlie", "jehd", "ernie"],
    20, // Generate 20 years worth of data
  );
